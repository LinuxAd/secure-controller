...
##@ Development
.PHONY: tidy
tidy: ## Tidy go mod file and pull deps.
	go mod tidy

.PHONY: manifests
manifests: controller-gen tidy ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
...
load:
	kind load docker-image $(IMG) || minikube image load $(IMG)
...
.PHONY: deploy
deploy: manifests kustomize load ## Deploy controller to the K8s cluster specified in ~/.kube/config.
